...
All logic is delegated to RAGService in kid_agent/services/rag/service.py.
"""
...
from kid_agent.services.rag.service import DEFAULT_KB_BASE_DIR, RAGService
    ...
    **kwargs,
    ...
    if kb_base_dir is None:
        try:
            ...
            from kid_agent.multi_user.knowledge_access import resolve_for_rag

            resource = resolve_for_rag(kb_name)
            if resource is not None:
                kb_base_dir = str(resource.base_dir)
                kb_name = resource.name
            elif not get_current_user().is_admin:
