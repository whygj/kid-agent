...
class ProgressTracker:
    ...
    def _notify(self, progress: dict):
        ...
        from kid_agent.runtime.mode import is_server

        if is_server():
            try:
                from kid_agent.api.utils.progress_broadcaster import ProgressBroadcaster
    ...
    def update(
        ...
        try:
            ...
            fallback_logger = logging.getLogger("kid_agent.ProgressTracker")
            prefix = f"[{self.task_id}]" if self.task_id else ""
        ...
        if self.task_id:
            try:
                from kid_agent.api.utils.task_log_stream import get_task_stream_manager
