...
from kid_agent.services.rag.factory import DEFAULT_PROVIDER, normalize_provider_name
from kid_agent.services.rag.file_routing import FileTypeRouter
...
def _get_embedding_fingerprint() -> tuple[str, int] | None:
    ...
    try:
        from kid_agent.services.embedding import get_embedding_config
...
def _reconcile_embedding_flags(knowledge_bases: dict, base_dir: Path | None = None) -> bool:
    ...
    * No match, but the KB has a stored ``embedding_model`` that differs
    ...
    """
    from kid_agent.services.rag.embedding_signature import signature_from_embedding_config
    from kid_agent.services.rag.index_versioning import (
...
class KnowledgeBaseManager:
    ...
    def __init__(self, base_dir="./data/knowledge_bases"):
        ...
        from kid_agent.services.pocketbase_client import is_pocketbase_enabled
    ...
    def _load_config(self) -> dict:
        ...
        if self.config_file.exists():
            try:
                ...
                from kid_agent.services.rag.index_versioning import list_kb_versions
    ...
    def update_kb_status(
        ...
        if status == "ready":
            ...
            try:
                from kid_agent.services.rag.embedding_signature import (
    ...
    def set_default(self, name: str):
        ...
        try:
            from kid_agent.services.config import get_kb_config_service
