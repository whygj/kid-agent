...
class TutorBotManager:
    ...
    async def start_bot(self, bot_id: str, config: BotConfig | None = None) -> TutorBotInstance:
        ...
        from kid_agent.services.tutorbot.model_runtime import (
        ...
        from kid_agent.tutorbot.config.schema import ExecToolConfig
        from kid_agent.tutorbot.providers.kid_agent_adapter import create_kid_agent_provider
        from kid_agent.tutorbot.session.manager import SessionManager

        llm_config = resolve_tutorbot_llm_config(config)
        provider = create_kid_agent_provider(llm_config)
        bus = MessageBus()
        ...
        # -- Heartbeat --------------------------------------------------------
        from kid_agent.tutorbot.heartbeat import HeartbeatService
    ...
    async def _outbound_router(self, bot_id: str, bus: Any, instance: TutorBotInstance) -> None:
        ...
        """
        try:
            from kid_agent.events.event_bus import Event, EventType, get_event_bus
            from kid_agent.tutorbot.bus.events import OutboundMessage as _OMsg
    ...
    def _build_channel_manager(
        ...
        *,
        ...
        from kid_agent.tutorbot.config.schema import ChannelsConfig
    ...
    async def send_message(
        ...
        if instance.channel_manager and response:
            from kid_agent.tutorbot.bus.events import OutboundMessage
