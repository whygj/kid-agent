...
from kid_agent.core.stream import StreamEvent, StreamEventType
from kid_agent.services.path_service import get_path_service
from kid_agent.services.session.protocol import SessionStoreProtocol
...
def _llm_selection_dict(value: Any) -> dict[str, str] | None:
    from kid_agent.services.model_selection import LLMSelection
...
class TurnRuntimeManager:
    ...
    def __init__(self, store: SessionStoreProtocol | None = None) -> None:
        from kid_agent.services.session import get_session_store
    ...
    async def start_turn(self, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        try:
            from kid_agent.capabilities.request_contracts import validate_capability_config
        ...
        if llm_selection:
            try:
                from kid_agent.multi_user.model_access import apply_allowed_llm_selection
            ...
            from kid_agent.multi_user.context import get_current_user
            from kid_agent.multi_user.model_access import redacted_model_access
        ...
        if llm_selection:
            from kid_agent.services.config import get_model_catalog_service
            from kid_agent.services.model_selection import (
                LLMSelection,
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        try:
            from kid_agent.agents.notebook import NotebookAnalysisAgent
            try:
                from kid_agent.book.context import build_book_context
            except ImportError:
                build_book_context = None
            from kid_agent.core.context import Attachment, UnifiedContext
            from kid_agent.runtime.orchestrator import ChatOrchestrator
            from kid_agent.services.memory import get_memory_service
            from kid_agent.services.model_selection.runtime import (
                activate_llm_selection,
            )
            from kid_agent.services.model_selection.runtime import (
                reset_llm_selection as reset_active_llm_selection,
            )
            from kid_agent.services.notebook import get_notebook_manager
            from kid_agent.services.session.context_builder import ContextBuilder
            from kid_agent.services.skill import get_skill_service
            ...
            from kid_agent.services.storage import get_attachment_store
            ...
            from kid_agent.utils.document_extractor import extract_documents_from_records
            ...
            from kid_agent.multi_user.context import get_current_user
            from kid_agent.multi_user.paths import get_admin_path_service
            from kid_agent.multi_user.skill_access import assigned_skill_ids
            from kid_agent.services.skill.service import SkillService
